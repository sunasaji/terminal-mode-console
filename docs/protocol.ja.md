...
# even-terminal 互換 HTTP API メモ(v0.10.4 時点)
...
1. `@evenrealities/even-terminal@0.10.4`(MIT ライセンスで npm 公開)のソースの読解
...
- 素の Express 5、既定ポート **3456**。バインド先: 上流 ≤v0.8.1 は `app.listen(PORT, "0.0.0.0")`。v0.10.x は検出したプライベート LAN アドレス + ループバックに待ち受ける(`--tailscale` / `-i` / `--lan` で選択)。terminal-mode-console は既定でループバックのみ(`HOST` env で変更可)
- 独自プロトコル・署名・証明書ピン留め・デバイス登録は **一切なし**
- WebSocket は表示 API には使わない(アプリは REST + SSE をポーリング)。内部連携専用: codex app-server(既定 8765)と、v0.10.x 以降は実験的な `claude-sync` transport(メインポート上)。どちらも相互運用には不要
...
- 不一致は全エンドポイントで `401 {"error":{"code":"auth_failed","message":"Unauthorized"}}`(v0.10.x は `{code,message}` 形式で包む。v0.8.1 は `{"error":"Unauthorized"}` の文字列だった)
- トークンは `~/.even-terminal/instances/<pid>.json`(mode 0600)に平文保存

## 3. CORS

寛容な CORS。上流 ≤v0.8.1 は無条件で有効(`app.use(cors())`)。v0.10.x は `--allow-cors` による opt-in(既定オフ)。terminal-mode-console は常に `Access-Control-Allow-Origin: *` を返す。OPTIONS 応答:
...
| POST     | `/permission-response`                   | `{sessionId, decision}` decision: `allow`/`allowAlways`/`deny`。v0.10.x は提示されていない決定を拒否する: 未知セッション → `404 {"error":"Session not found"}`、保留中の要求が無いセッション → `400 {"error":"Permission decision was not offered or no permission request is pending"}`(v0.8.1 は常に 200 で受理していた)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
- バージョン差分。ここに書いたのは v0.10.4 時点の観察で、上流の更新で変わりうる。v0.8.1 からの主な変更: 401 と `/info` 失敗時の `{code,message}` エラー形式、`/info` の `extra.expose` フィールド、`/permission-response` の検証強化(404/400)、実験的な `claude-sync` provider 名の追加(`claude`/`codex` と併せて受理される。実 `claude` CLI を WebSocket + ループバック hook エンドポイントで包む内部機構で、terminal-mode-console は再実装しない)、CORS が opt-in 化(`--allow-cors`)、`0.0.0.0` ではなくプライベート LAN アドレスへのバインド。
  自分の環境で確かめるのが確実。
...
- LAN アドレス待ち受け + 単一トークンなので **同一 LAN の他端末から到達可能**(上流 ≤v0.8.1 は `0.0.0.0`、v0.10.x はプライベート LAN アドレス、terminal-mode-console は既定でループバックのみ)
- CORS が有効な場合(v0.8.1 までは無条件、v0.10.x は `--allow-cors` で opt-in、terminal-mode-console は常時有効)、ブラウザで開いた任意のページから `?token=` 付きで叩ける
